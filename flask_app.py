...
def displaydatabyyear(substance, year):
    ...
    return render_template(Html_template,type = year, result=data.get_data_by_state(substance, year))
...
def displaydatabystate(substance, state):
    ...
    Html_template='display'+str(substance).title()+'Data.html'
    return render_template(Html_template, type = state.title(), result=search_result)
